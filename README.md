...
3. Watch the dashboard — parsing is synchronous, AI analysis runs in the background and the status badge updates from "pending" to "ok" within a few seconds.
...
- **Why batching**: the free tier's context window and rate limits are too tight to send an entire uploaded file in one request, so parsed log entries are split into batches of 50 and sent concurrently (up to 5 batches in flight at a time). Results from all batches are concatenated into one report.
  - **Known limitation**: because each batch is analyzed independently, a pattern spread thinly across many batches (e.g. a slow, low-and-slow brute force) won't be caught — only patterns visible within a single batch of 50 entries. A paid tier or a larger-context model would let this run in fewer, bigger batches.
...
Sample Apache/Nginx-style combined access logs for testing are in [`examples/`](examples/):
- `sample-access.log` / `sample-access.txt` / `sample-access.json` — same small sample, in each of the accepted upload extensions. Parsing is extension-agnostic (every upload is scanned line-by-line against the combined log format regardless of extension), so the `.json` file is not structured JSON — it's the same log lines as the others, just named to exercise that upload path.
- `big-sample-access.log` — larger file, useful for exercising batching/concurrency in the AI analysis step
...
- Uploaded file bytes are stored directly in Postgres (`BYTEA`) rather than in object storage — simpler for a prototype, but not ideal much past a few MB per file.
- No versioned DB migrations — the schema is a single idempotent script applied on every boot.

## Future: cloud deployment

Not currently deployed. The intended path would be:
- **Client**: deploy to [Vercel](https://vercel.com) (it's already a standard Next.js app), pointing `NEXT_PUBLIC_API_BASE` at the deployed API.
- **Server + database**: containerize the Go API and deploy to a platform like Fly.io, Render, or GCP Cloud Run, backed by a managed Postgres instance (e.g. Cloud SQL, Supabase, or Neon).
- Swap the hardcoded CORS origin in `server/cors.go` and the default DB DSN in `server/db/db.go` for environment-driven config before deploying.
